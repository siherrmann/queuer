...
func TestGetWorker(t *testing.T) {
	...
	t.Run("Successfully get worker created by the new queuer", func(t *testing.T) {
		...
		retrievedWorker, err := q.GetWorker(q.worker.RID)
		...
		assert.Equal(t, q.worker.RID, retrievedWorker.RID, "expected retrieved worker RID to match the original worker RID")
	})
	...
	t.Run("Returns error for non-existent worker", func(t *testing.T) {
		...
		retrievedWorker, err := q.GetWorker(nonExistentWorkerRid)
...
func TestGetWorkers(t *testing.T) {
	...
	t.Run("Return empty slice for non existant lastId", func(t *testing.T) {
		...
		workers, err := q.GetWorkers(1000, 10)
